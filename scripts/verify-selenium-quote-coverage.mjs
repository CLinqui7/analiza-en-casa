...
console.log(`failed=${requireExecuted ? failed.length : 0}`);
console.log(`pending_runtime=${requireExecuted ? 0 : failed.length}`);
...
for (const action of failed) console.log(`${requireExecuted ? 'FAILED' : 'PENDING_RUNTIME'} ${action.action_id}: missing, invalid, non-PASS, or stale git SHA`);
for (const entry of orphanEvidence) console.log(`ORPHAN_EVIDENCE ${entry.action_id}`);
if (coverage !== 100 || missingInUi.length || orphanUiIds.length || orphanEvidence.length || (requireExecuted && failed.length)) process.exitCode = 1;
