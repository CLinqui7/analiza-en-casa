...
const req = [
  ...
  'app/main.js',
  'supabase/migrations',
  'references/video-audit/CHAPTERS.json',
  'references/video-audit/MASTER_EVENT_MANIFEST.csv',
  'references/video-audit/chapters',
  'video-audit-reviews',
  'docs/VIDEO_AUDIT_SETUP.md',
...
for (const file of files) {
  ...
  if (
    st.size < 2 * 1024 * 1024 &&
    /\.(md|txt|json|js|mjs|sql|yml|yaml|ps1|bat|html|css)$/i.test(file)
  ) {
...
const chapters = fs.existsSync(chaptersPath)
  ? fs
      ...
      .filter((x) => x.startsWith('CH') && fs.statSync(path.join(chaptersPath, x)).isDirectory())
      .sort()
  : [];
...
for (const ch of chapters) {
  ...
  const count = (sub, ext) => {
    ...
    return fs.existsSync(p)
      ? fs.readdirSync(p).filter((x) => x.toLowerCase().endsWith(ext)).length
      : 0;
  };
  ...
  for (const needed of [
    ...
    'event_manifest.csv',
    'transcript_raw.txt',
    'CODEX_ANALYZE_THIS_CHAPTER.md',
    'CODEX_IMAGE_BATCHES.md',
  ]) {
...
const report = {
  ...
  detail_crops: crops,
  exact_clips: clips,
  event_contact_sheets: eventSheets,
  safety_contact_sheets: safetySheets,
  errors,
  warnings,
};
fs.mkdirSync(path.join(root, 'docs/qa'), { recursive: true });
fs.writeFileSync(
  path.join(root, 'docs/qa/REPOSITORY_PREFLIGHT.json'),
  JSON.stringify(report, null, 2),
);
fs.writeFileSync(
  path.join(root, 'docs/qa/REPOSITORY_PREFLIGHT.md'),
