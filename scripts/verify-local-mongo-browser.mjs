...
try {
  ...
  assert.equal(await loadError.count(), 0, await loadError.count() ? await loadError.innerText() : 'Workspace rendered');
  ...
  await patientDialog.getByRole('textbox', { name: 'Dirección obligatorio', exact: true }).fill('Dirección ficticia QA');
  await patientDialog.getByLabel('Comentarios relevantes de la dirección').fill('Referencia QA');
  let rejectedSaves = 0;
  await page.route('**/api/patients', async route => {
    if (route.request().method() !== 'POST') return route.continue();
    rejectedSaves++;
    await route.fulfill({ status: 503, contentType: 'application/json', body: JSON.stringify({ error: 'Error de guardado simulado para QA.' }) });
  });
  ...
  assert.ok(!(await (await context.request.get(`${base}/api/workspace`)).json()).patients.some(row => row.documentId === 'QA-SAVE-FAILURE'));
  await page.screenshot({ path: '.local/mongo-verification/patient-save-failure.png' });
  await page.unroute('**/api/patients');
  for (const route of ['patients', 'hospitalizations', 'quotes', 'receivables', 'insurance', 'agenda', 'catalogs/operational', 'clinical/balance', 'clinical/administrations', 'nursing-team', 'reports/visits-goals', 'changes', 'inventory/kardex', 'inventory/movements']) {
    await page.goto(`${base}/${route}`);
    await page.waitForLoadState('networkidle');
    if (route === 'patients') assert.ok(await page.getByText(workspace.patients[0].fullName, { exact: true }).count() > 0, 'Mongo patient must render in React');
    await page.screenshot({ path: `.local/mongo-verification/${route.replaceAll('/', '-')}.png` });
    const createAction = {patients:'PATIENT-CREATE',hospitalizations:'HOSPITALIZATION-CREATE',quotes:'QUOTE-CREATE','nursing-team':'NURSE-ACCOUNT-CREATE','catalogs/operational':'CONFIGURATION-CREATE','clinical/administrations':'MEDICATION-ADMINISTER','reports/visits-goals':'HOME-VISIT-CREATE'}[route];
    if (createAction) {
      ...
      if (await button.count() && await button.isEnabled()) {
        ...
        await page.screenshot({path:`.local/mongo-verification/${route.replaceAll('/','-')}-dialog.png`});
        await page.setViewportSize({width:390,height:844});
        await page.screenshot({path:`.local/mongo-verification/${route.replaceAll('/','-')}-mobile-dialog.png`});
  ...
  const result = { verifiedAt: new Date().toISOString(), base, provider: 'mongodb', patients: workspace.patients.length, hospitalizations: workspace.hospitalizations.length, quotes: workspace.quotes.length, saveFailureVerified: true, browserErrors: errors };
