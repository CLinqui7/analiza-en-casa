...
const blockerText = (row) => String(row.blocker ?? '');
const resolvable = rows.filter(
  (row) => row.status === 'MISSING' || (row.status === 'MIGRATED_PARTIAL' && !blockerText(row).includes('CLIENT')),
);
...
for (const row of resolvable) console.log(`GAP ${row.route_id}: ${row.status} — ${blockerText(row) || 'none recorded'}`);
