...
export async function migrationPlan() {
  ...
  const names = (await readdir(directory)).filter((name) => /^\d+_[a-z_]+\.json$/.test(name)).sort();
  ...
  return Promise.all(names.map(async (version) => {
    ...
    return { ...JSON.parse(raw), version, sha256: createHash('sha256').update(raw).digest('hex') };
  }));
}
...
export async function migrateMongo(environment = process.env) {
  ...
  assert.ok(environment.MONGODB_URI && environment.MONGODB_DB, 'Private MongoDB configuration required');
  ...
  try {
    ...
    for (const migration of plan) {
      ...
      if (previous) {
        assert.equal(previous.sha256, migration.sha256, 'Applied MongoDB migration changed; add a new version');
      ...
      await ledger.insertOne({ _id: migration.version, sha256: migration.sha256, appliedAt: new Date() }, { writeConcern: { w: 'majority' } });
      applied.push(migration.version);
    }
    return { status: 'PASS', applied, skipped, seededUsers: 0 };
  } finally {
    try {
      if (acquired) await client.db(environment.MONGODB_DB).collection(prefix + 'schemaMigrationLocks').updateOne(
        { _id: 'deployment', owner }, { $set: { released: true, completedAt: new Date() } }, { writeConcern: { w: 'majority' } },
      );
