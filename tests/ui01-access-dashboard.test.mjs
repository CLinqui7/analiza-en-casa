...
test('UI01 login keeps fixture credentials explicitly inside demo mode and supports keyboard focus/errors', async () => {
  const source = await readFile(loginForm, 'utf8');

  assert.match(source, /const demoMode = isDemoAuthMode\(\)/);
  assert.match(source, /useState\(\(\) => \(demoMode \? 'admin@demo\.local' : ''\)\)/);
  assert.match(source, /useState\(\(\) => \(demoMode \? 'demo-admin' : ''\)\)/);
