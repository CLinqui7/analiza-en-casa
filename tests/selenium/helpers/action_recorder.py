...
def record_pass(action_id: str, test_id: str, started_at: float, url: str) -> None:
    ...
    data['results'].append({'action_id': action_id, 'test_id': test_id, 'status': 'PASS', 'git_sha': sha, 'functional_fingerprint': fingerprint, 'executed_at': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'duration_ms': int((time.time()-started_at)*1000), 'url': url})
