...
class Quotes(unittest.TestCase):
    ...
    def test_ch04_referral_tags_and_bounded_service_catalog_persist_without_pricing_rules(self) -> None:
        ...
        self.assertEqual([option.text for option in services.options], ['Seleccione un servicio', 'Servicio de atención disponible'])
        self.pass_('QUOTE-INVENTORY-ONLY', 'SEL-CH04-QUOTE-GENERAL', inventory_started)
        business_partner_started = time.time(); Select(self.action('QUOTE-BUSINESS-PARTNER')).select_by_visible_text('Socio de negocios A')
        service_started = time.time(); services.select_by_visible_text('Servicio de atención disponible')
        ...
        self.w.until(EC.visibility_of_element_located((By.XPATH, "//tbody/tr[contains(.,'Servicio de atención disponible')]")))
        ...
        self.w.until(lambda _: any(q['summary'] == marker for q in self.snapshot()['quotes']))
        quote = next(q for q in self.snapshot()['quotes'] if q['summary'] == marker)
        persisted_item = next(item for item in quote['items'] if item['name'] == 'Servicio de atención disponible')
        self.assertEqual(persisted_item['businessPartnerLabel'], 'Socio de negocios A')
    ...
    def test_ch05_service_and_medication_catalogs_search_reset_and_persist_manual_amounts(self) -> None:
        ...
        service_option = self.w.until(EC.element_to_be_clickable((By.XPATH, "//*[@aria-label='Resultados de servicios']//*[@role='option' and normalize-space()='Servicio de atención disponible']")))
        ...
        self.assertEqual(self.field('Concepto').get_attribute('value'), 'Servicio de atención disponible')
        Select(self.action('QUOTE-BUSINESS-PARTNER')).select_by_visible_text('Socio de negocios A')
        ...
        self.assertFalse(self.d.find_elements(By.XPATH, "//tbody/tr[contains(.,'Servicio de atención disponible')]"))
        ...
        medication_option = self.w.until(EC.element_to_be_clickable((By.XPATH, "//*[@aria-label='Resultados de medicamentos']//*[@role='option' and normalize-space()='Medicamento disponible']")))
        ...
        self.assertEqual(self.field('Concepto').get_attribute('value'), 'Medicamento disponible')
        medication_partner_started = time.time(); Select(self.action('QUOTE-MEDICATION-BUSINESS-PARTNER')).select_by_visible_text('Socio de negocios B')
        self.fill('Cantidad', '1'); self.fill('Precio manual', '9'); self.click('QUOTE-ITEM-ADD')
        self.w.until(EC.visibility_of_element_located((By.CSS_SELECTOR, '.quote-processing')))
        self.w.until(EC.visibility_of_element_located((By.XPATH, "//tbody/tr[contains(.,'Medicamento disponible')]")))
        self.click('QUOTE-CREATE-SUBMIT'); self.w.until(EC.url_to_be(f'{BASE}/quotes'))
        quote = next(item for item in self.snapshot()['quotes'] if item['summary'] == marker)
        self.d.refresh(); self.quotes_ready(); self.open_detail(quote['id'])
        body = self.d.find_element(By.TAG_NAME, 'body').text
        self.assertIn('Servicio de atención disponible', body); self.assertIn('Medicamento disponible', body)
        self.assertEqual([(item['name'], item['unitPrice']) for item in self.quote_data(quote['id'])['items']], [('Servicio de atención disponible', 12), ('Medicamento disponible', 9)])
    ...
    def test_ch06_supply_study_and_fee_catalogs_persist_only_manual_amounts(self) -> None:
        ...
        supply_partner_started = time.time(); Select(self.action('QUOTE-SUPPLY-BUSINESS-PARTNER')).select_by_visible_text('Socio de negocios A')
        self.assertEqual(Select(self.action('QUOTE-SUPPLY-BUSINESS-PARTNER')).first_selected_option.text, 'Socio de negocios A')
        supply_search_started = time.time(); self.fill('Buscar insumos', 'sin coincidencia')
        self.w.until(EC.visibility_of_element_located((By.XPATH, "//*[@role='status' and normalize-space()='No results found']")))
        self.fill('Buscar insumos', 'INS-001')
        supply_option = self.w.until(EC.element_to_be_clickable((By.XPATH, "//*[@aria-label='Resultados de insumos']//*[@role='option' and contains(.,'Insumo disponible')]")))
        self.assertTrue(supply_option.is_displayed())
        supply_select_started = time.time(); supply_option.click()
        self.assertIn('INS-001', self.field('Concepto').get_attribute('value'))
        self.fill('Cantidad', '2'); self.fill('Precio manual', '12'); self.click('QUOTE-ITEM-ADD')
        self.w.until(EC.visibility_of_element_located((By.XPATH, "//tbody/tr[contains(.,'INS-001')]")))
        ...
        study_partner_started = time.time(); Select(self.action('QUOTE-STUDY-BUSINESS-PARTNER')).select_by_visible_text('Socio de negocios B')
        self.assertEqual(Select(self.action('QUOTE-STUDY-BUSINESS-PARTNER')).first_selected_option.text, 'Socio de negocios B')
        study_search_started = time.time(); self.fill('Buscar estudios', 'hemoglobina')
        study_option = self.w.until(EC.element_to_be_clickable((By.XPATH, "//*[@aria-label='Resultados de estudios']//*[@role='option' and normalize-space()='Estudio de hemoglobina disponible']")))
        self.assertTrue(study_option.is_displayed())
        study_select_started = time.time(); study_option.click()
        self.assertEqual(self.field('Concepto').get_attribute('value'), 'Estudio de hemoglobina disponible')
        self.fill('Cantidad', '1'); self.fill('Precio manual', '9'); self.click('QUOTE-ITEM-ADD')
        self.w.until(EC.visibility_of_element_located((By.XPATH, "//tbody/tr[contains(.,'Estudio de hemoglobina disponible')]")))

        self.w.until(EC.element_to_be_clickable((By.XPATH, "//button[@role='tab' and normalize-space(.)='Honorarios']"))).click()
        fee_partner_started = time.time(); Select(self.action('QUOTE-FEE-BUSINESS-PARTNER')).select_by_visible_text('Socio de negocios A')
        self.assertEqual(Select(self.action('QUOTE-FEE-BUSINESS-PARTNER')).first_selected_option.text, 'Socio de negocios A')
        fee_service_started = time.time(); Select(self.action('QUOTE-FEE-SERVICE-CATALOG')).select_by_visible_text('Seguimiento disponible')
        self.assertEqual(self.field('Concepto').get_attribute('value'), 'Seguimiento disponible')
        Select(self.action('QUOTE-FEE-DOCTOR-SELECT')).select_by_index(1)
        self.fill('Cantidad', '1'); self.fill('Honorario médico (manual)', '15'); self.click('QUOTE-ITEM-ADD')
        self.w.until(EC.visibility_of_element_located((By.XPATH, "//tbody/tr[contains(.,'Seguimiento disponible')]")))
        self.click('QUOTE-CREATE-SUBMIT'); self.w.until(EC.url_to_be(f'{BASE}/quotes'))
        quote = next(item for item in self.snapshot()['quotes'] if item['summary'] == marker)
        self.d.refresh(); self.quotes_ready(); self.open_detail(quote['id'])
