...
class Patients(unittest.TestCase):
 ...
 def create_full_patient_fixture(s):
  ...
  s.click('PATIENT-SAVE')
  s.w.until(EC.invisibility_of_element_located((By.CSS_SELECTOR,'[data-action-id="PATIENT-SAVE"]')))
  ...
  s.assert_detail_contains('Paciente Fixture Edit Selenium','POL-FIXTURE-001','Titular Fixture Original','Contacto Fixture Principal','Contacto Fixture Secundario','Dirección Fixture Original','Referencia Fixture Original')
 def open_create(s):
  s.d.get(BASE+'/patients'); s.click('PATIENT-CREATE'); s.w.until(EC.visibility_of_element_located((By.CSS_SELECTOR,'[data-action-id="PATIENT-SAVE"]')))
 def fill_required(s,name,document,phone):
  Select(s.a('PATIENT-DOCUMENT-TYPE')).select_by_value('OTHER'); s.fill('Documento',document); s.fill('Nombre completo',name); s.fill('Fecha de nacimiento','04/20/1985'); s.field('Femenino').click(); s.fill('Teléfono celular',phone); s.fill('Empresa','Empresa Selenium'); s.fill('Dirección','Calle Selenium 123'); s.fill('Comentarios relevantes de la dirección','Referencia Selenium')
 def test_create_and_validation(s):
  t=time.time(); s.open_create(); s.assertIn('/patients',s.d.current_url); s.assertNotIn('edit=',s.d.current_url); s.assertTrue(s.field('Nombre completo').is_displayed()); s.assertTrue(s.a('PATIENT-SAVE').is_displayed()); s.fill_required('Paciente Creado Selenium','CREATE-SEL-001','70003333'); s.click('PATIENT-SAVE'); s.w.until(EC.invisibility_of_element_located((By.CSS_SELECTOR,'[data-action-id="PATIENT-SAVE"]'))); s.w.until(EC.visibility_of_element_located((By.XPATH,"//*[contains(text(),'Registro sintético agregado')]"))); search=s.a('PATIENT-SEARCH'); search.send_keys('CREATE-SEL-001'); s.w.until(EC.visibility_of_element_located((By.XPATH,"//*[contains(text(),'Paciente Creado Selenium')]"))); s.d.refresh(); s.a('PATIENT-SEARCH').send_keys('CREATE-SEL-001'); s.w.until(EC.visibility_of_element_located((By.XPATH,"//*[contains(text(),'Paciente Creado Selenium')]"))); s.pass_('PATIENT-CREATE','SEL-PAT-CREATE',t); s.pass_('PATIENT-SAVE','SEL-PAT-CREATE',t)
 def test_contacts(s):
 ...
 def test_address(s):
  s.open_create(); s.fill('Nombre completo','Paciente Dirección Selenium'); s.fill('Teléfono celular','70005555'); s.fill('Dirección','Calle Selenium 123'); s.fill('Comentarios relevantes de la dirección','Referencia Selenium'); s.fill('Ubicación geográfica','13.7000,-89.2000'); s.fill('Pegar enlace','https://example.test/ubicacion'); t=time.time(); s.click('PATIENT-ADDRESS-CLEAR');
  for label in ('Dirección','Comentarios relevantes de la dirección','Ubicación geográfica','Pegar enlace'): s.assertEqual(s.field(label).get_attribute('value'),'')
  s.assertEqual(s.field('Nombre completo').get_attribute('value'),'Paciente Dirección Selenium'); s.assertEqual(s.field('Teléfono celular').get_attribute('value'),'70005555'); s.pass_('PATIENT-ADDRESS-CLEAR','SEL-PAT-ADDRESS',t); s.fill('Pegar enlace','https://example.test/selenium-map'); t=time.time(); before=s.d.current_url; s.click('PATIENT-ADDRESS-IMPORT'); s.w.until(EC.visibility_of_element_located((By.XPATH,"//*[contains(text(),'No se encontraron coordenadas inequívocas')]"))); s.assertEqual(s.field('Pegar enlace').get_attribute('value'),'https://example.test/selenium-map'); s.assertEqual(s.d.current_url,before); s.pass_('PATIENT-ADDRESS-IMPORT','SEL-PAT-ADDRESS',t)
 ...
 def test_document_type(s):
  s.open_create(); t=time.time(); Select(s.a('PATIENT-DOCUMENT-TYPE')).select_by_value('PASSPORT'); s.fill('Documento','PAS-SEL-001'); s.fill('Nombre completo','Paciente Documento Selenium'); s.fill('Teléfono celular','70007777'); Select(s.a('PATIENT-DOCUMENT-TYPE')).select_by_value('DUI'); s.assertEqual(s.field('Documento').get_attribute('value'),''); s.assertEqual(s.field('Nombre completo').get_attribute('value'),'Paciente Documento Selenium'); s.pass_('PATIENT-DOCUMENT-TYPE','SEL-PAT-DOCUMENT',t)
 def test_contact_document_pair_persists_through_reload_and_edit(s):
  ...
  partial_started=time.time(); s.click('PATIENT-SAVE')
 ...
 def test_edit(s):
  ...
  s.fill('Comentarios relevantes de la dirección','Referencia Selenium Editada')
 ...
 def test_export(s):
  ...
  exported=time.time(); s.d.find_element(By.CSS_SELECTOR,'.patient-more-actions summary').click(); s.click('PATIENT-EXPORT-CSV'); active=s.wait_download('pacientes-activos.csv')
  ...
  s.d.find_element(By.CSS_SELECTOR,'.patient-more-actions summary').click()
