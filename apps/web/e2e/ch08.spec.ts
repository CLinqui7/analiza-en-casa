...
async function loginToDetail(page: import('@playwright/test').Page, email: string, password: string) {
...
async function expectReadOnlyProfile(page: import('@playwright/test').Page, email: string, password: string) {
  await loginToDetail(page, email, password);
  await expect(page.getByRole('heading', { name: 'case-demo-001' })).toBeVisible();
  const before = await page.evaluate(() => [localStorage.getItem('analiza.en.casa.workspace.v3.hospitalizations'), localStorage.getItem('analiza.en.casa.workspace.v3.auditEntries')]);
  await expect(page.locator('[data-action-id="HOSPITALIZATION-ADMIN-PROFILE-OPEN"]')).toHaveCount(0);
  await expect(page.locator('[data-action-id="HOSPITALIZATION-ADMIN-PROFILE-SAVE"]')).toHaveCount(0);
  await expect(page.locator('[data-action-id="HOSPITALIZATION-ADMIN-PROFILE-CANCEL"]')).toHaveCount(0);
  await page.reload();
  await expect.poll(() => page.evaluate(() => [localStorage.getItem('analiza.en.casa.workspace.v3.hospitalizations'), localStorage.getItem('analiza.en.casa.workspace.v3.auditEntries')])).toEqual(before);
}

test('CH08 stores an execution administrative profile and keeps cancellation non-persistent', async ({ page }) => {
  await login(page);
  ...
  const caseId = await page.evaluate(() => {
    const records = JSON.parse(localStorage.getItem('analiza.en.casa.workspace.v3.hospitalizations') ?? '[]');
    return records.find((item: { nextAction?: string }) => item.nextAction === 'CH08 perfil administrativo')?.id as string;
  });
  expect(caseId).toBeTruthy();
  await page.goto(`/hospitalizations/${caseId}`);
  await expect(page.getByRole('heading', { name: 'Perfil administrativo de ejecución · PIC' })).toBeVisible();

  await page.getByRole('button', { name: 'Editar perfil administrativo' }).click();
  const profile = page.getByRole('dialog', { name: `Perfil administrativo de ejecución: ${caseId}` });
  ...
  await expect(page.getByText('Perfil administrativo de ejecución guardado.', { exact: true })).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Editar perfil administrativo' }).click();
  const reopened = page.getByRole('dialog', { name: `Perfil administrativo de ejecución: ${caseId}` });
...
test('CH08 ADMIN can open the administrative profile on a direct hospitalization detail route', async ({ page }) => {
  await loginToDetail(page, 'admin@demo.local', 'demo-admin');
  await page.getByRole('button', { name: 'Editar perfil administrativo' }).click();
  await expect(page.getByRole('dialog', { name: 'Perfil administrativo de ejecución: case-demo-001' })).toBeVisible();
  await expect(page.locator('[data-action-id="HOSPITALIZATION-ADMIN-PROFILE-SAVE"]')).toBeVisible();
});

test('CH08 DOCTOR can open the administrative profile on a direct hospitalization detail route', async ({ page }) => {
...
test('CH08 NURSE reads the direct detail without profile controls or mutation', async ({ page }) => {
  await expectReadOnlyProfile(page, 'nurse@demo.local', 'demo-nurse');
});

test('CH08 FINANCE reads the direct detail without profile controls or mutation', async ({ page }) => {
  await expectReadOnlyProfile(page, 'finance@demo.local', 'demo-finance');
});

test('CH08 AUDITOR reads the direct detail without profile controls or mutation', async ({ page }) => {
