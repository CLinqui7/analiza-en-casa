...
async function login(page: import('@playwright/test').Page, next: string, email = 'admin@demo.local', password = 'demo-admin') {
...
test('B3 persists multiple admission and discharge periods without accepting attachment bytes', async ({ page }) => {
  ...
  await expect(dialog.getByText('Los archivos privados de hospitalización siguen bloqueados')).toBeVisible();
...
test('B3 creates persisted multi-day 6h shifts and keeps Puntual blocked', async ({ page }) => {
  ...
  expect(new Date(persisted[0].endsAt).getTime() - new Date(persisted[0].startsAt).getTime()).toBe(6 * 60 * 60 * 1000);
});
...
test('B3 lets NURSE persist an overnight 8h shift', async ({ page }) => {
  ...
  expect(new Date(persisted.endsAt).getTime() - new Date(persisted.startsAt).getTime()).toBe(8 * 60 * 60 * 1000);
});
...
test('B3 prevents DOCTOR from altering shifts or audit entries', async ({ page }) => { await expectAgendaWriteDenied(page, 'doctor@demo.local', 'demo-doctor'); });
test('B3 prevents INVENTORY from altering shifts or audit entries', async ({ page }) => { await expectAgendaWriteDenied(page, 'inventory@demo.local', 'demo-inventory'); });
test('B3 prevents FINANCE from altering shifts or audit entries', async ({ page }) => { await expectAgendaWriteDenied(page, 'finance@demo.local', 'demo-finance'); });
test('B3 prevents AUDITOR from altering shifts or audit entries', async ({ page }) => { await expectAgendaWriteDenied(page, 'auditor@demo.local', 'demo-auditor'); });
