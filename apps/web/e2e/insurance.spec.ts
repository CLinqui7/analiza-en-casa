...
test('insurance records manual observations, searches, filters and preserves related modules', async ({
  ...
  await dialog
    .getByLabel('Observación / respuesta')
    .fill('Respuesta administrativa sintética para QA.');
  ...
  await expect(page.locator('summary')).toContainText(
    'Respuesta administrativa sintética para QA.',
  );
  await page.reload();
  await expect(page.locator('summary')).toContainText(
    'Respuesta administrativa sintética para QA.',
  ...
  for (const term of [
    ...
    '00000000',
    'quote demo 001',
    'aseguradora de demostracion',
  ]) {
    await search.fill(term);
    await expect(page.getByRole('link', { name: 'quote-demo-001' }).first()).toBeVisible();
  }
  await search.fill('sin resultado QA');
  await expect(page.getByText('Sin resultados', { exact: true })).toBeVisible();
  await page.getByRole('button', { name: 'Limpiar búsqueda' }).click();
  await expect(search).toHaveValue('');
  await page.getByLabel('Estado observado').first().selectOption('SENT_TO_INSURER');
  await expect(page.getByRole('link', { name: 'quote-demo-001' }).first()).toBeVisible();
  ...
  await update
    .getByLabel('Observación / respuesta')
    .fill('Segunda observación sintética append-only.');
  await update.getByRole('button', { name: 'Registrar actualización' }).click();
  await page
    .getByText('Segunda observación sintética append-only.', { exact: true })
    .first()
    .click();
  await expect(
...
test('insurance cancellation, invalid quote, safe channels and role guards are enforced', async ({
  ...
  await login(admin);
  await admin.goto('/insurance?quote=quote-demo-001');
  ...
  for (const role of [
    ...
    await expect(page.getByRole('button', { name: 'Registrar actualización' })).toHaveCount(
      role.write ? 2 : 0,
    );
    await context.close();
  }
  for (const role of [
    { email: 'nurse@demo.local', password: 'demo-nurse' },
    { email: 'inventory@demo.local', password: 'demo-inventory' },
  ]) {
    const context = await browser.newContext();
    const page = await context.newPage();
    await login(page, role.email, role.password);
