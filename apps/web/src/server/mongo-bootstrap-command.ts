...
async function seedSyntheticWorkspace(
  database: Awaited<ReturnType<typeof mongoDatabase>>,
  ...
  for (const resource of demoNursingResources)
    await upsert('nursingResources', resource.id, resource);
  for (const hospitalization of demoHospitalizations)
    await upsert('hospitalizations', hospitalization.id, {
      ...hospitalization,
      version: 1,
      createdAt: createdAt.toISOString(),
      updatedAt: createdAt.toISOString(),
  ...
  for (const shift of demoShifts)
    await upsert('shifts', shift.id, { ...shift, createdAt: createdAt.toISOString() });
  for (const item of demoCatalogItems) {
    await upsert('catalogItems', item.id, {
      ...item,
      skuNormalized: item.sku.replace(/\s/g, '').toUpperCase(),
    });
  }
  for (const entry of demoOperationsConfiguration) {
    await upsert('configurationEntries', entry.id, entry);
  }
  const medicationItemId = 'catalog-demo-medication-qa';
  await upsert('inventoryMovements', 'movement-demo-medication-qa', {
    ...
    user: 'bootstrap',
    idempotencyKey: 'seed-medication-entry-qa-001',
  });
  await upsert('inventoryBalances', 'inventory-balance-demo-medication-central', {
    id: 'inventory-balance-demo-medication-central',
    itemId: medicationItemId,
    warehouseId: 'central',
    quantity: 100,
  });
  ...
  const seededQuote = {
    ...
    rootQuoteId: sourceQuote.id,
    originalQuoteId: sourceQuote.id,
  };
  await upsert('quotes', seededQuote.id, {
    ...seededQuote,
    recordVersion: 1,
    updatedAt: createdAt.toISOString(),
  });
  const request = {
    id: 'insurance-demo-001',
    quoteId: seededQuote.id,
    patientId: seededQuote.patientId,
    insurer: 'Aseguradora de demostración',
    status: 'INSURER_REVIEW',
    createdAt: createdAt.toISOString(),
    updatedAt: createdAt.toISOString(),
    lastNote: 'Solicitud sintética en revisión para validar el tablero.',
  };
  await upsert('insuranceRequests', request.id, request);
  await upsert('insuranceEvents', 'insurance-event-demo-001', {
    id: 'insurance-event-demo-001',
    requestId: request.id,
    status: request.status,
    date: createdAt.toISOString(),
    note: request.lastNote,
    idempotencyKey: 'seed-insurance-event-demo-001',
    actorUserId: 'bootstrap',
  });
  await upsert('auditEvents', 'audit-seed-demo-001', {
    id: 'audit-seed-demo-001',
    actorUserId: 'bootstrap',
    action: 'SYNTHETIC_SEED_APPLIED',
...
export async function runMongoBootstrap(
  ...
  Readonly<{ dryRun: boolean; initializedAdmin: boolean; seededRecords: number; indexes: number }>
> {
  ...
  if (dryRun) {
    ...
    return {
      ...
      seededRecords: 0,
      indexes: report.plannedIndexes.length,
    };
  }
