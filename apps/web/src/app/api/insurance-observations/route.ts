...
import { MongoInsuranceRepository } from '@/server/mongo-quotes';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await new MongoInsuranceRepository(database).recordObservation(actor, await request.json());
