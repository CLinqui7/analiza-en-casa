...
export async function GET(request: NextRequest) {
  try {
    ...
    const actor = await new MongoAuthService(mongoAuthStore(database)).requireSession(request.cookies.get(sessionCookieName)?.value);
    return NextResponse.json({ quotes: await new MongoQuoteRepository(database).listWithVersions(actor) }, { headers: { 'Cache-Control': 'no-store' } });
