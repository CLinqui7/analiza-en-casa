...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const expectedVersion = body && typeof body === 'object' && !Array.isArray(body) ? (body as { expectedVersion?: unknown }).expectedVersion : undefined;
    const quote = await new MongoQuoteRepository(database).send(actor, (await context.params).id, expectedVersion as number);
