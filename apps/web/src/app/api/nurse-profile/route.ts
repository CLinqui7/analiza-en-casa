...
import { AuthenticationError, csrfHeaderName, sessionCookieName } from '@/server/auth-service';
  ...
  return NextResponse.json(
    {
      error:
        status === 409
          ? 'El perfil cambió en otra pestaña. Recarga los datos antes de guardar.'
          : status === 400
            ? 'Revisa los campos del perfil de enfermería.'
            : status === 401
              ? 'Inicia sesión para continuar.'
              : status === 403
                ? 'No tienes permiso para editar este perfil.'
                : 'No pudimos guardar ni cargar el perfil. Intenta nuevamente.',
    },
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(await backend.nurseProfile.get(actor), { headers: privateHeaders });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      await backend.nurseProfile.save(actor, await boundedJson(request, 64 * 1024)),
