...
export default function AuditPage() {
  ...
  function exportAudit() {
    const csv = toCsv([
      ['Fecha', 'Acción', 'Referencia'],
      ...auditEntries.map((entry) => [entry.at, entry.action, entry.subject]),
    ]);
    ...
    <div className="page-stack">
      ...
      <Panel>
        <div className="table-heading">
          <h2>Eventos</h2>
          <StatusTag>{auditEntries.length} registros</StatusTag>
        </div>
        {auditEntries.length ? (
            ...
            <table>
              ...
              <tbody>
                {auditEntries.map((entry) => (
                  <tr key={entry.id}>
                    <td>{new Date(entry.at).toLocaleString('es-SV')}</td>
                    <td>{entry.action}</td>
                    <td>
          ...
          <EmptyState detail="Las mutaciones persistidas aparecerán aquí." title="Sin eventos" />
        )}
      </Panel>
    </div>
  );
