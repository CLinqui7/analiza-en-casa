...
export default function HospitalizationDetailPage() {
  ...
  const { clinicalDocuments, hospitalizations, loading, patients, providerMode, quotes, updateHospitalization, vitalReadings } = useWorkspace();
  const { can } = useAuth();
  ...
  const saveProfile = (event: FormEvent<HTMLFormElement>) => {
    ...
    updateHospitalization({
      ...
      administrativeProfile: {
        healthManager: value('healthManager'), referredBy: value('referredBy'), revenueType: value('revenueType'), type: value('type'),
        startDate: value('startDate'), durationDays: value('durationDays'), paymentMethod: value('paymentMethod'), insurer: value('insurer'),
        requestType: value('requestType'), majorCategory: value('majorCategory'), subcategory: value('subcategory'),
        originatingHospital: value('originatingHospital'), patientClass: value('patientClass'),
      },
    });
  ...
  return <div className="page-stack">
    ...
    <AdministrativeProfilePanel canWrite={can('cases:write')} hospitalization={hospitalization} onOpen={() => { setProfileMessage(null); setProfileOpen(true); }} providerMode={providerMode} />
    {profileMessage ? <p className="notice success" role="status">{profileMessage}</p> : null}
    ...
    {mockProfileEnabled ? <Dialog description="Conserva campos administrativos observados. No aplica tarifas, cobertura, impuestos ni decisiones de aseguradora." footer={<><Button className="button-secondary" data-action-id="HOSPITALIZATION-ADMIN-PROFILE-CANCEL" onClick={closeProfile} type="button">Cancelar</Button><Button data-action-id="HOSPITALIZATION-ADMIN-PROFILE-SAVE" form="administrative-profile-form" type="submit">Guardar</Button></>} onClose={closeProfile} open={profileOpen} title={`Perfil administrativo de ejecución: ${hospitalization.id}`}>
      <form className="form-grid" id="administrative-profile-form" onSubmit={saveProfile}>
        <label>Health manager<input defaultValue={profile?.healthManager ?? ''} name="healthManager" /></label><label>Referido por<input defaultValue={profile?.referredBy ?? ''} name="referredBy" /></label><label>Tipo Revenue<input defaultValue={profile?.revenueType ?? ''} name="revenueType" /></label><label>Tipo<input defaultValue={profile?.type ?? ''} name="type" /></label>
        <label>Fecha de inicio<input defaultValue={profile?.startDate ?? hospitalization.startDate} name="startDate" type="date" /></label><label>Días de duración<input defaultValue={profile?.durationDays ?? ''} inputMode="numeric" min="0" name="durationDays" type="number" /></label><label>Forma de pago<input defaultValue={profile?.paymentMethod ?? ''} name="paymentMethod" /></label><label>Aseguradora<input defaultValue={profile?.insurer ?? hospitalization.insurer ?? ''} name="insurer" /></label>
        <label>Tipo de solicitud<input defaultValue={profile?.requestType ?? ''} name="requestType" /></label><label>Categoría mayor<input defaultValue={profile?.majorCategory ?? ''} name="majorCategory" /></label><label>Subcategoría<input defaultValue={profile?.subcategory ?? ''} name="subcategory" /></label><label>Hospital de origen<input defaultValue={profile?.originatingHospital ?? ''} name="originatingHospital" /></label><label>Clase de paciente<input defaultValue={profile?.patientClass ?? ''} name="patientClass" /></label>
      </form>
    </Dialog> : null}
  </div>;
}
