...
export default function AgendaPage() {
  ...
  const shiftRows = (items: typeof visibleShifts) => items.map((shift) => <tr key={shift.id}><td>{new Date(shift.startsAt).toLocaleString('es-SV')}</td><td>{new Date(shift.endsAt).toLocaleString('es-SV')}</td><td>{nursingResources.find((resource) => resource.id === shift.resourceId)?.displayName ?? 'No disponible'}</td><td>{patients.find((patient) => patient.id === shift.patientId)?.fullName ?? 'Sin asignar'}</td><td>{statusLabel[shift.status]}</td><td>{shift.note ?? '—'}</td></tr>);
  return <div className="page-stack">
    ...
    <Dialog description="Puede crear una serie de días sin duplicados ni colisiones del mismo recurso. Puntual sigue pendiente de definición del cliente." footer={<><Button className="button-secondary" onClick={close} type="button">Cerrar</Button><Button data-action-id="AGENDA-SHIFT-SAVE" form="shift-form" type="submit">Guardar</Button></>} onClose={close} open={open} title="Crear turno a paciente"><form className="form-grid" id="shift-form" noValidate onSubmit={form.handleSubmit(submit)}>
