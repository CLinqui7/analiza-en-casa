...
export default function PatientDetailPage() {
  ...
  return <div className="page-stack">
    ...
    <section className="two-column">
      <Panel><h2>Identificación</h2><dl className="definition-list"><div><dt>Nombre</dt><dd>{patient.fullName}</dd></div><div><dt>Documento</dt><dd>{patient.documentType}: {patient.documentId}</dd></div><div><dt>Fecha de nacimiento</dt><dd>{patient.birthDate ?? 'Sin dato'}</dd></div><div><dt>Sexo</dt><dd>{patient.sex ?? 'Sin dato'}</dd></div><div><dt>Empresa</dt><dd>{patient.company ?? 'Sin dato'}</dd></div><div><dt>Jubilado</dt><dd>{patient.retired ? 'Sí' : 'No'}</dd></div><div><dt>Tipo de sangre</dt><dd>{patient.bloodType ?? 'Sin dato'}</dd></div><div><dt>Estado civil</dt><dd>{patient.civilStatus ?? 'Sin dato'}</dd></div><div><dt>Nacionalidad</dt><dd>{patient.nationality ?? 'Sin dato'}</dd></div><div><dt>Ocupación</dt><dd>{patient.occupation ?? 'Sin dato'}</dd></div></dl></Panel>
      <Panel><h2>Contacto y seguro</h2><dl className="definition-list"><div><dt>Teléfono celular</dt><dd>{patient.phone ?? 'Sin dato'}</dd></div><div><dt>Teléfono de casa</dt><dd>{patient.homePhone ?? 'Sin dato'}</dd></div><div><dt>Correo</dt><dd>{patient.email ?? 'Sin dato'}</dd></div><div><dt>Aseguradora</dt><dd>{patient.insurance?.status === 'INSURED' ? patient.insurance.insurer : 'Paciente regular'}</dd></div><div><dt>Póliza</dt><dd>{patient.insurance?.policyNumber ?? 'Sin dato'}</dd></div><div><dt>Titular</dt><dd>{patient.insurance?.holderFullName ?? 'Sin dato'}</dd></div><div><dt>Fecha efectiva</dt><dd>{patient.insurance?.effectiveDate ?? 'Sin dato'}</dd></div></dl></Panel>
      <Panel><h2>Contactos</h2>{patient.contacts?.length ? <ul>{patient.contacts.map((contact) => <li key={contact.id}>{contact.fullName || 'Sin nombre'} · {contact.phone || 'Sin teléfono'} · {contact.isPrimary ? 'Principal' : 'Secundario'}</li>)}</ul> : <p>Sin contactos registrados.</p>}</Panel>
      <Panel><h2>Dirección</h2><dl className="definition-list"><div><dt>Dirección</dt><dd>{patient.address?.line ?? 'Sin dato'}</dd></div><div><dt>Referencia</dt><dd>{patient.address?.comments ?? 'Sin dato'}</dd></div><div><dt>Coordenadas</dt><dd>{patient.address?.coordinates ?? 'Sin dato'}</dd></div><div><dt>Enlace de ubicación</dt><dd>{patient.address?.locationUrl ?? 'Sin dato'}</dd></div></dl></Panel>
    </section>
    <Link className="text-link" data-action-id="PATIENT-BACK-TO-LIST" href="/patients">Volver a pacientes</Link>
    <Dialog description="La actualización persiste en el proveedor configurado y añade evidencia de auditoría." footer={<><Button className="button-secondary" data-action-id="PATIENT-EDIT-CANCEL" onClick={close} type="button">Cancelar</Button><Button data-action-id="PATIENT-EDIT-SUBMIT" form="patient-edit-form" type="submit">Guardar cambios</Button></>} onClose={close} open={open} title="Editar paciente"><form className="form-grid" id="patient-edit-form" noValidate onSubmit={form.handleSubmit(submit)}><label>Nombre completo<input {...form.register('fullName')} />{form.formState.errors.fullName ? <span className="field-error">{form.formState.errors.fullName.message}</span> : null}</label><label>Tipo de documento<select {...form.register('documentType')} onChange={(event) => { form.setValue('documentType', event.target.value as PatientForm['documentType']); form.setValue('documentId', ''); form.clearErrors('documentId'); }}><option value="DUI">DUI</option><option value="PASSPORT">Pasaporte</option><option value="OTHER">Otro documento</option></select></label><label>Número de documento<input {...form.register('documentId')} inputMode={documentType === 'DUI' ? 'numeric' : undefined} onChange={(event) => { const value = documentType === 'DUI' ? maskDui(event.target.value) : event.target.value; form.setValue('documentId', value, { shouldDirty: true }); form.clearErrors('documentId'); }} placeholder={documentRules[documentType ?? 'DUI'].mask ?? 'Identificador'} />{form.formState.errors.documentId ? <span className="field-error">{form.formState.errors.documentId.message}</span> : null}</label><label>Teléfono de demo<input {...form.register('phone')} /></label><label>Aseguradora<input {...form.register('insurer')} /></label></form></Dialog>
  </div>;
}
