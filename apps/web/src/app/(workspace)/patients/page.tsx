...
import { ageFromBirthDate, documentRules, findDuplicatePatient, maskDui, searchPatients, toCsv, validateDocument } from '@analiza/domain';
...
const patientFormSchema = z.object({
  ...
  civilStatus: z.string().trim(), nationality: z.string().trim(), occupation: z.string().trim(), triageStatus: z.string().trim(), botmakerConsent: z.boolean(),
  insurance: z.object({
...
export default function PatientsPage() {
  ...
  const form = useForm<PatientForm>({
    ...
    defaultValues: {
      ...
      homePhone: '', email: '', retired: false, bloodType: '', civilStatus: '', nationality: '', occupation: '', triageStatus: '',
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      bloodType: editingPatient.bloodType ?? '', civilStatus: editingPatient.civilStatus ?? '', nationality: editingPatient.nationality ?? '', occupation: editingPatient.occupation ?? '', triageStatus: editingPatient.triageStatus ?? '',
  ...
  function exportPatientsCsv() {
    const content = toCsv([['Documento', 'Nombre completo', 'Edad', 'Empresa', 'Triage', 'Notif. Botmaker', 'Estado'], ...visiblePatients.map((patient) => [patient.documentId, patient.fullName, String(ageFromBirthDate(patient.birthDate) ?? '—'), patient.company ?? '—', patient.triageStatus || 'Sin clasificar', patient.notifications?.botmakerConsent ?? true ? 'Sí' : 'No', patient.status === 'ACTIVE' ? 'Activo' : 'Inactivo'])]);
  ...
  function onSubmit(values: PatientForm) {
    ...
    const patient: Patient = {
      ...
      civilStatus: values.civilStatus || undefined, nationality: values.nationality || undefined, occupation: values.occupation || undefined, triageStatus: values.triageStatus || undefined,
  ...
  return (
    <div className="page-stack">
      ...
      <Panel><div className="table-heading"><h2>Resultados</h2><div><StatusTag>{visiblePatients.length} visibles</StatusTag><label> Mostrar <select data-action-id="PATIENT-PAGE-SIZE" onChange={(event) => { setPageSize(Number(event.target.value)); setPage(1); }} value={pageSize}><option value="5">5</option><option value="10">10</option><option value="25">25</option></select></label></div></div>{visiblePatients.length ? <div className="table-wrap"><table><thead><tr><th>Acción</th><th aria-sort={sort === 'documentId' ? (direction === 1 ? 'ascending' : 'descending') : 'none'}><Button aria-label="Ordenar por documento" data-action-id="PATIENT-SORT-DOCUMENT" onClick={() => { setDirection(sort === 'documentId' ? (direction * -1) as 1 | -1 : 1); setSort('documentId'); setPage(1); }} type="button">Documento {sort === 'documentId' ? (direction === 1 ? '↑' : '↓') : '↕'}</Button></th><th aria-sort={sort === 'fullName' ? (direction === 1 ? 'ascending' : 'descending') : 'none'}><Button aria-label="Ordenar por nombre completo" data-action-id="PATIENT-SORT-NAME" onClick={() => { setDirection(sort === 'fullName' ? (direction * -1) as 1 | -1 : 1); setSort('fullName'); setPage(1); }} type="button">Nombre completo {sort === 'fullName' ? (direction === 1 ? '↑' : '↓') : '↕'}</Button></th><th>Edad</th><th>Empresa</th><th>Triage</th><th>Notif. Botmaker</th><th>Estado</th></tr></thead><tbody>{pageRows.map((patient) => <tr key={patient.id}><td><div className="header-actions"><Link data-action-id="PATIENT-DETAIL-NAVIGATE" href={`/patients/${patient.id}`}>Detalle</Link>{can('patients:write') ? <Button data-action-id={patient.status === 'ACTIVE' ? 'PATIENT-INACTIVATE' : 'PATIENT-REACTIVATE'} onClick={() => updatePatient({ ...patient, status: patient.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE' })} type="button">{patient.status === 'ACTIVE' ? 'Inactivar' : 'Reactivar'}</Button> : null}</div></td><td>{patient.documentType}: {patient.documentId}</td><td>{patient.fullName}</td><td>{ageFromBirthDate(patient.birthDate) ?? '—'}</td><td>{patient.company ?? '—'}</td><td>{patient.triageStatus || 'Sin clasificar'}</td><td>{patient.notifications?.botmakerConsent ?? true ? 'Sí' : 'No'}</td><td><StatusTag tone={patient.status === 'ACTIVE' ? 'success' : 'neutral'}>{patient.status === 'ACTIVE' ? 'Activo' : 'Inactivo'}</StatusTag></td></tr>)}</tbody></table></div> : <EmptyState detail="Cambie el criterio o agregue un registro sintético." title="Sin resultados" />}</Panel>
      ...
      <Dialog description="Los datos administrativos sintéticos se validan y persisten en el proveedor configurado." footer={<><Button className="button-secondary" data-action-id={editingPatient ? "PATIENT-EDIT-CANCEL" : "PATIENT-CREATE-CANCEL"} onClick={closeDialog} type="button">Cancelar</Button><Button data-action-id={editingPatient ? "PATIENT-EDIT-SUBMIT" : "PATIENT-CREATE-SUBMIT"} form="patient-form" type="submit">{editingPatient ? 'Guardar cambios' : 'Guardar registro'}</Button></>} onClose={closeDialog} open={isOpen || linkedDialogOpen || editDialogOpen} title={editingPatient ? 'Editar paciente' : 'Agregar paciente'}>
        <form className="form-grid" id="patient-form" noValidate onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <fieldset><legend>Estados administrativos</legend><label>Triage administrativo<input {...form.register('triageStatus')} placeholder="Sin clasificar" /></label><p className="field-help">Dato administrativo observado; no se calcula ni interpreta clínicamente.</p></fieldset>
        </form>
      </Dialog>
