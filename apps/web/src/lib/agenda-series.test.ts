...
describe('agenda series', () => {
  it('creates one shift per selected day and derives 6h/8h endings', () => {
    ...
    expect(
      buildShiftSeries({
        ...
        status: 'SCHEDULED',
        existing: [],
        idFor: (date) => `shift-${date}`,
      }).map((shift) => shift.id),
    ).toEqual(['shift-2026-09-01', 'shift-2026-09-03']);
  });
    ...
    const existing: Shift[] = [
      {
        id: 'existing',
        resourceId: 'nurse-1',
        startsAt: new Date('2026-09-01T08:00').toISOString(),
        endsAt: new Date('2026-09-01T14:00').toISOString(),
        status: 'SCHEDULED',
      },
    ];
    expect(() =>
      buildShiftSeries({
        dates: ['2026-09-02', '2026-09-02'],
        resourceId: 'nurse-1',
        startTime: '08:00',
        endTime: '14:00',
        status: 'SCHEDULED',
  ...
  it('preserves exact preset durations across midnight', () => {
    expect(endForPreset('2026-09-01', '20:00', 'SIX_HOURS')).toEqual({
      endTime: '02:00',
      endDayOffset: 1,
    });
    ...
    expect(new Date(shift.endsAt).getTime() - new Date(shift.startsAt).getTime()).toBe(
      6 * 60 * 60 * 1000,
    );
  });
});
