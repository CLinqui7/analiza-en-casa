...
export class MockDataProvider implements DataProvider {
  ...
  async load(): Promise<WorkspaceSnapshot> {
    ...
    try {
      ...
      if (!Array.isArray(parsed.patients) || !Array.isArray(parsed.auditEntries) || !Array.isArray(parsed.shifts)) throw new Error('invalid');
      return parsed;
    } catch {
...
export class SupabaseDataProvider implements DataProvider {
  ...
  async load(): Promise<WorkspaceSnapshot> {
    ...
    const [patients, vitalReadings, nursingResources, nurseHours, inventoryMovements, shifts, auditEntries] = await Promise.all([
      ...
      client.from('inventory_movements').select('*'), client.from('shifts').select('*'),
      client.from('audit_log').select('*'),
    ]);
    const failed = [patients, vitalReadings, nursingResources, nurseHours, inventoryMovements, shifts, auditEntries]
  ...
  async save(snapshot: WorkspaceSnapshot): Promise<void> {
    ...
    const results = await Promise.all([
      ...
      client.from('inventory_movements').upsert(snapshot.inventoryMovements), client.from('shifts').upsert(snapshot.shifts), client.from('audit_log').upsert(snapshot.auditEntries),
    ]);
