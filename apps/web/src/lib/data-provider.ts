...
export class MockDataProvider implements DataProvider {
  ...
  async load(): Promise<WorkspaceSnapshot> {
    ...
    try {
      ...
      if (!Array.isArray(parsed.patients) || !Array.isArray(parsed.auditEntries) || !Array.isArray(parsed.shifts) || !Array.isArray(parsed.hospitalizations)) throw new Error('invalid');
      return parsed;
    } catch {
...
export class SupabaseDataProvider implements DataProvider {
  ...
  async load(): Promise<WorkspaceSnapshot> {
    ...
    const [patients, vitalReadings, nursingResources, nurseHours, inventoryMovements, shifts, hospitalizations, auditEntries] = await Promise.all([
      ...
      client.from('inventory_movements').select('*'), client.from('shifts').select('*'), client.from('hospitalizations').select('*'),
      client.from('audit_log').select('*'),
    ]);
    const failed = [patients, vitalReadings, nursingResources, nurseHours, inventoryMovements, shifts, hospitalizations, auditEntries]
  ...
  async save(snapshot: WorkspaceSnapshot): Promise<void> {
    ...
    const results = await Promise.all([
      ...
      client.from('inventory_movements').upsert(snapshot.inventoryMovements), client.from('shifts').upsert(snapshot.shifts), client.from('hospitalizations').upsert(snapshot.hospitalizations), client.from('audit_log').upsert(snapshot.auditEntries),
    ]);
