...
`CH17-E0008` was opened. The report route now renders only its six observed section labels as local tabs. Selecting a tab changes only an empty state and resets on reload without changing audit storage.
