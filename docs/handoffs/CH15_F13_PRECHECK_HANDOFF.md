...
`CH15-Q004` leaves the authoritative source, type/category taxonomy, flag effects, write roles, fields, validation, audit, and clinical or financial consequences undefined. Playwright passed 30/30 on isolated port 4182. Selenium source is action-specific and remains `PENDING_RUNTIME` because Python is unavailable. The implementation is awaiting its immutable commit SHA.
