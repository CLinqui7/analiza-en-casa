...
F01–F03 remain the only safe local surfaces: empty anatomy only, with no patient, case, insurance, clinical, audit, document, range, print, navigation, or mutation data path. Their certification stays `UNVERIFIED` until a writable Git stabilization commit binds the already-recorded Playwright result to an immutable SHA.
