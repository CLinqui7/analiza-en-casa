...
- `MONGODB_SEED_ORGANIZATION_ID` se usa sólo por el comando opcional `--seed-synthetic`; la
  semilla es idempotente, identificable y contiene únicamente pacientes y operación ficticios.
- El segundo factor por WhatsApp requiere `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_ACCESS_TOKEN`,
  `WHATSAPP_GRAPH_API_VERSION`, `WHATSAPP_PORTAL_OTP_TEMPLATE` y
  `WHATSAPP_PORTAL_TEMPLATE_LANGUAGE`. Todas son privadas y la plantilla sólo recibe el OTP.
...
auditado. Sin la opción de semilla no inserta pacientes ni datos clínicos.
Use `--seed-synthetic` con `MONGODB_SEED_ORGANIZATION_ID` para cargar el conjunto QA mínimo; no
sobrescribe registros existentes y nunca debe apuntar a una organización con datos reales.
