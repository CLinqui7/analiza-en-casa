...
function renderHealthReportDetail(state, store, ui, caseId) {
  ...
  const activeAssessmentTab = ui.healthReportAssessmentTab || "history";
  ...
  const assessmentTabs = [["history","Antecedentes y evaluaciones"],["vitals","Signos vitales"],["profiles","Perfiles clínicos"],["evolutions","Notas de evolución"],["consultations","Interconsultas"],["nursing","Notas de enfermería"],["logs","Bitácoras"]];
  const reportDocuments = documents.filter((item) => ["HEALTH_REPORT", "CLINICAL_EVOLUTION", "CARE_PLAN"].includes(item.type));
  const reportedBackground = [...new Set(reportDocuments.flatMap((item) => item.content?.background || []).map(String).filter(Boolean))];
  const reportedAllergies = [...new Set(reportDocuments.flatMap((item) => item.content?.allergies || []).map(String).filter(Boolean))];
  let body = "";
  if (activeTab === "assessment") {
    const assessmentNav = `<nav class="tabs report-tabs" aria-label="Secciones de evaluación clínica">${assessmentTabs.map(([key,label]) => `<button class="tab ${activeAssessmentTab === key ? "active" : ""}" data-action="set-health-report-assessment-tab" data-tab="${key}">${esc(label)}</button>`).join("")}</nav>`;
    if (activeAssessmentTab === "history") {
      body = `${assessmentNav}${card("Antecedentes", `<div class="details-grid"><div><span>Antecedentes personales patológicos</span><strong>${esc(reportedBackground.join(" · ") || "No registrado")}</strong></div><div><span>Hábitos personales</span><strong>No registrado</strong></div><div><span>Antecedentes quirúrgicos</span><strong>No registrado</strong></div><div><span>Antecedentes familiares</span><strong>No registrado</strong></div><div><span>Tarjeta de vacunas</span><strong>No registrado</strong></div></div>`)}${card("Alergias", `<p>${reportedAllergies.length ? esc(reportedAllergies.join(" · ")) : "No registrado"}</p><p class="info-callout">El catálogo, la reacción y la política para actualizar alergias requieren confirmación clínica; esta vista no infiere ni modifica antecedentes.</p>`)}`;
    } else if (activeAssessmentTab === "vitals") {
      body = `${assessmentNav}${card("Gráficos", `<p class="info-callout">La tabla es la fuente autoritativa. La visualización no clasifica ni infiere umbrales clínicos.</p>`)}${card("Registros clínicos de signos vitales", table(["Fecha","TA","FC","FR","SpO₂","Temp","Dolor","Profesional"], vitals.map((vital) => `<tr><td>${formatDate(vital.recordedAt,true)}</td><td>${vital.systolic}/${vital.diastolic}</td><td>${vital.heartRate}</td><td>${vital.respiratoryRate}</td><td>${vital.spo2}%</td><td>${vital.temperature} °C</td><td>${vital.pain}</td><td>${esc(vital.authorName || "—")}</td></tr>`), {compact:true}))}${card("Signos cargados por el paciente", `<p>Sin registros de paciente en el período seleccionado.</p>`)}`;
    } else if (activeAssessmentTab === "profiles") {
      body = `${assessmentNav}<div class="details-grid"><div><span>Diagnóstico documentado</span><strong>${esc(profile ? `${profile.diagnosisCode} · ${profile.diagnosisLabel}` : "Sin perfil clínico")}</strong></div><div><span>Grupo diagnóstico</span><strong>${esc(profile?.diagnosisGroup || "—")}</strong></div><div><span>Triage</span><strong>${esc(profile?.triage || "No asignado")}</strong></div><div><span>Grupo / subgrupo</span><strong>${esc([profile?.profileGroup, profile?.profileSubgroup].filter(Boolean).join(" · ") || "—")}</strong></div></div>${card("Dispositivos", table(["Dispositivo","Fecha","Calibre","Motivo","Frecuencia","Observaciones"], (profile?.devices || []).map((device) => `<tr><td>${esc(device.deviceType)}</td><td>${formatDate(device.date)}</td><td>${esc(device.gauge || "—")}</td><td>${esc(device.reason || "—")}</td><td>${esc(device.changeFrequency || "—")}</td><td>${esc(device.observations || "—")}</td></tr>`), {compact:true}))}`;
    } else if (activeAssessmentTab === "nursing") {
      const noteQuery = String(ui.healthReportNursingSearch || "").trim().toLocaleLowerCase("es");
      const filteredNotes = notes.filter((note) => !noteQuery || [note.authorName, note.text, note.status, note.shift].some((value) => String(value || "").toLocaleLowerCase("es").includes(noteQuery)));
      const notesPageSize = 10;
      const notesTotalPages = Math.max(1, Math.ceil(filteredNotes.length / notesPageSize));
      const notesPage = Math.min(Math.max(1, Number(ui.healthReportNursingPage || 1)), notesTotalPages);
      const noteRows = filteredNotes.slice((notesPage - 1) * notesPageSize, notesPage * notesPageSize).map((note) => `<tr><td><input type="checkbox" data-action="toggle-health-report-note-print" data-id="${esc(note.id)}" ${ui.healthReportPrintNoteIds?.includes(note.id) ? "checked" : ""} aria-label="Incluir nota ${esc(note.id)} al imprimir"></td><td>${formatDate(note.createdAt,true)}</td><td>${esc(note.shift || "—")}</td><td>${esc(note.authorName || "—")}</td><td>${esc(note.text || "—")}</td><td>${badge(note.status)}</td><td>${note.status === "SIGNED" ? `<button class="row-action" data-action="open-clinical-correction" data-subject-type="NURSING_NOTE" data-id="${esc(note.id)}">Corrección auditada</button><button class="row-action" data-action="share-note" data-id="${esc(note.id)}">Compartir enlace seguro</button>` : "—"}</td></tr>`);
      const notePages = Array.from({length: notesTotalPages}, (_, index) => index + 1).map((page) => `<button data-action="health-report-nursing-page" data-page="${page}" class="${page === notesPage ? "active" : ""}">${page}</button>`).join("");
      body = `${assessmentNav}${card("Notas de enfermería", `<div class="filter-bar"><label class="search-field">${icon("search")}<input data-input="health-report-nursing-search" value="${esc(ui.healthReportNursingSearch || "")}" placeholder="Buscar nota, turno o profesional"></label></div>${table(["Imprimir","Fecha","Turno","Recurso","Nota clínica","Estado","Acciones"], noteRows, {compact:true})}<nav class="pagination" aria-label="Paginación de notas de enfermería"><button data-action="health-report-nursing-page" data-page="${notesPage - 1}" ${notesPage === 1 ? "disabled" : ""}>Anterior</button>${notePages}<button data-action="health-report-nursing-page" data-page="${notesPage + 1}" ${notesPage === notesTotalPages ? "disabled" : ""}>Siguiente</button></nav>`)}`;
    } else {
      const label = assessmentTabs.find(([key]) => key === activeAssessmentTab)?.[1] || "Sección";
      body = `${assessmentNav}${card(label, `<p class="info-callout">No hay registros estructurados para esta sección en el período seleccionado. La semántica y los permisos de captura siguen pendientes de contrato clínico.</p>`)}`;
    }
